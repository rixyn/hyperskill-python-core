...
# Required to pass the Hyperskill tests:
...
def remove_files_with_extension(extension, directory='.'):
...
def copy_files_with_extension(extension, target_dir, directory='.'):
    """Copy all files with the given extension to the target directory."""
    files_copied = False
    for file in os.listdir(directory):
        if os.path.splitext(file)[1] == f'.{extension}':
            ...
            if os.path.exists(target_path):
                response = input(
                    f"{file} already exists in this directory. Replace? (y/n) ")
                if response.lower() != 'y':
                    continue
...
def move_files_with_extension(extension, target_dir, directory='.'):
    """Move all files with the given extension to the target directory."""
    files_moved = False
    for file in os.listdir(directory):
        if os.path.splitext(file)[1] == f'.{extension}':
            source_path = os.path.join(directory, file)
    ...
    while True:
        ...
        if command == "quit":
            ...
            if len(commands) == 1:
                ...
                remove_files_with_extension(commands[1][1:], current_dir)
            else:
            ...
            if len(commands) != 3:
                ...
                if len(commands) >= 3 and commands[1].startswith('.'):
                    move_files_with_extension(
                        commands[1][1:], commands[2], current_dir)
                else:
